...
class PandaEEFVelocityController(object):

    def __init__(self):
        self.num_joints = 7
        # Get the URDF (you will need to write your own "load_urdf()" function)
        urdf_path = os.path.join( os.path.dirname(os.path.realpath(__file__)), './urdf/panda_arm_hand.urdf')
       # urdf_path= os.path.join(robosuite.models.assets_root, "bullet_data/sawyer_description/urdf/sawyer_intera.urdf")

        self.urdf_model = URDF.from_xml_string(open(urdf_path).read().encode('ascii'))  # encode to ascii solve the error
